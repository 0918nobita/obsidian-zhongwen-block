...
import TextLine from './components/TextLine.svelte';
import ZhText from './components/ZhText.svelte';
import NonZhText from './components/NonZhText.svelte';
import { splitIntoSegmentsPerLine, type Segment } from './split-sentence';
...
<div lang="zh-CN" data-always-display-pinyin={alwaysDisplayPinyin}>
    {#each segmentsPerLine as segments}
        <TextLine>
            {#each segments as segment}
                {#if segment.type === 'zh'}
                    {#await getPinyin(segment.zhChars) then zhChars}
                        <ZhText {zhChars} />
                    {/await}
                {:else}
